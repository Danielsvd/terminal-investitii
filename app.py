...
def format_num(val, is_pct=False):
    ...
    Previne eroarea 'TypeError: unsupported operand type(s) for *: 'NoneType' and 'float''
    """
...
def main():
    ...
    # ==================================================
        ...
        if hist is None or hist.empty:
            ...
            # Calcule defensive preț
            curr_price = subset['Close'].iloc[-1] if not subset.empty else 0
            start_price = subset['Close'].iloc[0] if not subset.empty else 0
            diff_val = curr_price - start_price
            diff_pct = (diff_val / start_price) * 100 if start_price else 0
            
            if len(hist) >= 2:
                prev_close = hist['Close'].iloc[-2]
                day_val = curr_price - prev_close
                day_pct = (day_val / prev_close) * 100 if prev_close else 0
            else:
                day_val = 0; day_pct = 0
